"""
NOTE: All code below is taken from langmuir.py in the PlasmaPy repository. Modifications were made to accelerate and improve the accuracy of Langmuir diagnostics. A suggested replacement for langmuir.py is below. (This file can be copied into your runtime environment. Search for "Leo" to find modifications.)
"""
...
def swept_probe_analysis(  # noqa: PLR0915
    ...
    """ LEO ADDITION vv
    import matplotlib.pyplot as plt
    plt.plot(probe_characteristic.bias, probe_characteristic.current)
    plt.plot(V_F, probe_characteristic.current[probe_characteristic.bias == V_F], 'go')
    plt.plot(V_P, probe_characteristic.current[probe_characteristic.bias == V_P], 'ro')
    plt.title(f"V_F: {V_F:.2f}, V_P: {V_P:.2f}")
    plt.plot()
    # ^^ LEO ADDITION """
    ...
    # LEO DEBUG
    """
    exponential_section.plot()
    import matplotlib.pyplot as plt
    plt.title("Exponential section first")
    plt.show()
    """
    # LEO
...
def get_floating_potential(probe_characteristic, return_arg=False):
    ...
    try:
        arg_V_F = np.nonzero(probe_characteristic.current < 0)[0][-1]  # Leo version
    except IndexError:
        arg_V_F = np.argmin(np.abs(probe_characteristic.current))  # original
...
def extract_exponential_section(probe_characteristic, T_e=None, ion_current=None):
    ...
    if T_e is not None:
        ...
        _filter = (probe_characteristic.bias > V_F + 0.2 * 1.5 * T_e / const.e) & (  # LEO MODIFICATION
