...
def build_xarrays(bias,current,positions,ramp_times,dt,langmuir_configs,ion_type):
    """
    Parameters
    ----------
    bias
    ...
    # Accounts for non Jan 2024 data
    if len(sweeping_configs) == 0:
        sweeping_configs = [langmuir_configs[0]]
                ...
                for iy, y in enumerate(y_vals):
                    ...
                    for shot in range(num_shots):
                        ...
                        for sweep, (first_index, last_index) in enumerate(sweep_windows):
                            ...
                            try:
                                # v_p_value, v_p_index = get_plasma_potential_slope(sorted_bias, sorted_current, v_f_index,
                                #                                              i_esat_index_max)
                            ...
                            try:
                                # i_electron_sat_value, i_esat_index_max = get_electron_isat_v_f(sorted_bias, sorted_current,v_f_index,get_V_P=True)
                                i_electron_sat_value, i_esat_index =get_electron_isat_curve_fit(sorted_bias,
                                                                                                sorted_current,
                                                                                                v_f_index,
                                                                                                v_p_index,
                                                                                                return_arg = True)
                                if i_electron_sat_value is None:
                            ...
                            if not isinstance(v_p_value, Quantity):
                                ...
                                try:
                                    t_e_value = get_te_v_p_vf(sorted_bias, sorted_current, v_f_index, v_p_index)
                                except Exception as e:
                                    t_e_value = np.nan
